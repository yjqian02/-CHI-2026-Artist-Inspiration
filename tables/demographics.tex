...
\begin{tabularx}{\linewidth}{
  >{\centering\arraybackslash}p{0.9cm}   % ID
  >{\centering\arraybackslash}p{0.9cm}   % Gender
  >{\centering\arraybackslash}p{1.0cm}   % Age
  >{\centering\arraybackslash}p{2.2 cm}   % Years making art
  >{\raggedright\arraybackslash}p{2.0cm} % Physical media
  >{\raggedright\arraybackslash}p{2.0cm} % Digital media
  >{\raggedright\arraybackslash}p{1.8cm} % Digital tools
  X                                      % AI tools (flex)
  >{\raggedright\arraybackslash}p{3.2cm} % Reference tools
}
\toprule
\textbf{ID} & \textbf{Gender} & \textbf{Age} & \textbf{Years Making Art} & \textbf{Physical Media} & \textbf{Digital Media} & \textbf{Digital Tools} & \textbf{AI Tools} & \textbf{Ref Tools} \\
\midrule
P1  &  &  &  &  &  &  &  &  \\
P2  &  &  &  &  &  &  &  &  \\
\bottomrule
