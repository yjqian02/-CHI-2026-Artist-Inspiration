...
\paragraph{(i) \underline{Visual editing}. } Traditional image editing with generative models is largely text-driven, but artists often find text prompts insufficient for capturing precise edits. We leveraged MagicQuill~\citep{liu2025magicquill} to combines direct visual manipulation with generative editing, allowing users to sketch or mark directly on an existing image (need uncovered in \S\ref{visualize}), reducing the burden of linguistic description while affording immediate, tangible control.
% over how images are 
...
To match artists' desires to observe other's work and translate others' stylistic tradition to their own (also a need from \S\ref{visualize}), 
...
Also based on \ref{visualize}, artists often wonder how a reference image might appear in a different medium. 
...
\subsubsection{Transparency, Learning, and Community.} The final set of tools help shape the creative and collaborative process groups of human artists. These tools help make the most salient parts of the input more transparent, such as highlighting visual information, obtaining simpler sketches, or summarizing the image using its key attributes. Transparent tools can also help learning and community building, through summarizing step-by-step tutorials of visual content to help artists learn new techniques and styles, and cultivating a broader ecosystem of open-source tools, feedback loops, and shared knowledge.
